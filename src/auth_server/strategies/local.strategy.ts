...
@Injectable() export class LocalStrategy extends PassportStrategy(Strategy) {
    ...
    async validate(email: string, password: string) { const user = await this.authService.login({ email, password, username: '' }); if (!user) { throw new UnauthorizedException('Invalid login credentials'); } return user; }
}
